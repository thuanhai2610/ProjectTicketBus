...
const Login = () => {
  ...
  const handleFacebookSuccess = async (credentialResponse) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3001/facebook-login",
        {
          credential: credentialResponse.credential,
        }
      );
      ...
      const errorMessage =
        error.response?.data?.message ||
        "Facebooklogin failed. Please try again.";
      setError(errorMessage);
    } finally {
      setSocialLoading(false);
    }
  ...
  return (
    <GoogleOAuthProvider
      clientId={
        "1055268521864-uqrdrd5mpqbeskmqe28gb2kk37050t4b.apps.googleusercontent.com"
      }
    >
      <div className="flex min-h-screen items-center justify-center bg-red-100">
        <div className="bg-white p-10 rounded-2xl shadow-2xl w-96 border border-gray-300">
          <h2 className="text-red-500 text-3xl font-semibold text-center mb-6">
            Welcome Back
          </h2>

          {error && <p className="text-red-500 text-center mb-4">{error}</p>}
          <FacebookLogin
            appId="1448901919613034"
            autoLoad={false}
